...
# from schemas.api.api_case import ApiCaseQuery
# from schemas.api.api_info import ApiQuery, ApiIds
# from schemas.api.api_report import TestReportQuery, TestReportDetailQuery
# from schemas.api.data_source import SourceQuery
# from schemas.api.env import EnvQuery, BindingDataSourceIn, BindingFuncIn
# from schemas.api.functions import FuncQuery
# from schemas.api.module import ModuleQuery
# from schemas.api.interface import InterfaceQuery
...
#     @classmethod
#     async def get_list(cls, params: InterfaceQuery):
...
#         return (
#             cls.query.filter(cls.enabled_flag == 1)
#             .with_entities(
#                 cls.id,
...
#         return (
#             cls.query.filter(cls.enabled_flag == 1, cls.product_id == product_id)
...
#         order_by = "{} {} {} {}".format(
#             params.order_field, sort_type, ",module_info.id", sort_type
#         )
#         u = aliased(User)
...
#             .outerjoin(
#                 ProjectInfo,
#                 and_(cls.project_id == ProjectInfo.id, ProjectInfo.enabled_flag == 1),
#             )
#             .outerjoin(User, User.id == cls.created_by)
#             .outerjoin(u, u.id == cls.updated_by)
#             .outerjoin(
#                 ApiInfo, and_(cls.id == ApiInfo.module_id, ApiInfo.enabled_flag == 1)
#             )
#             .group_by(cls.id)
...
#             cls.module_packages == module_packages, cls.enabled_flag == 1
#         ).all()
...
#         return cls.query.filter(
#             cls.packages_id == packages_id, cls.enabled_flag == 1
#         ).first()
...
#         elif (
#             params.order_field == "created_by_name"
#             or params.order_field == "updated_by_name"
#         ):
...
#         """获取用例名是否存在"""
#         stmt = select(cls.get_table_columns()).where(
#             cls.enabled_flag == 1, cls.name == name
#         )
#         return await cls.get_result(stmt)
...
#             return None
#         stmt = select(cls.get_table_columns()).where(
#             cls.id.in_(params.ids), cls.enabled_flag == 1
#         )
#         return await cls.get_result(stmt)
...
#         """统计用户创建的用例数量"""
#         stmt = select(func.count(cls.id).label("count")).where(
#             cls.enabled_flag == 1, cls.created_by == user_id
#         )
...
#         return cls.query.filter(
#             cls.creation_date.between(start_time, end_time), cls.enabled_flag == 1
#         )

#     @classmethod
#     def statistic_project_api_number(cls):
#         return (
#             cls.query.outerjoin(ProjectInfo, ProjectInfo.id == cls.project_id)
#             .outerjoin(User, User.id == cls.created_by)
#             .with_entities(
#                 ProjectInfo.name,
...
#                 cls.request.like(("%{}%".format(body_name))),
#                 cls.enabled_flag == 1,
#             )
#             .with_entities(cls.id)
#             .all()
...
#         stmt = (
#             select(
#                 cls.get_table_columns({"id"}),
#                 ApiInfo.name.label("api_name"),
#                 ApiInfo.method.label("api_method"),
#             )
#             .outerjoin(
#                 ApiInfo, and_(ApiInfo.id == cls.source_id, ApiInfo.enabled_flag == 1)
#             )
#             .where(
#                 cls.case_id == case_id, cls.version == version, cls.enabled_flag == 1
#             )
#             .order_by(cls.index.asc())
#         )
#         return await cls.get_result(stmt)
...
#                 func.any_value(func.concat("case_", ApiCase.id)).label(
#                     "to_relation_id"
#                 ),
...
#     @classmethod
#     async def get_relation_by_case_ids(
#         cls, case_ids: typing.List[typing.Union[str, int]]
#     ):
#         """获取关联关系，那些case 使用了对应的api"""
#         q = [cls.enabled_flag == 1]
#         stmt = (
...
#                 func.any_value(func.concat("api_", ApiInfo.id)).label(
#                     "from_relation_id"
#                 ),
#                 func.any_value(func.concat("case_", ApiCase.id)).label(
#                     "to_relation_id"
...
#         """根据套件ids查询用例"""
#         stmt = select(cls.get_table_columns()).where(
#             cls.id.in_(ids), cls.enabled_flag == 1
#         )
#         return await cls.get_result(stmt)
...
#         """根据套件名称查询用例"""
#         stmt = select(cls.get_table_columns()).where(
#             cls.name == name, cls.enabled_flag == 1
#         )
#         return await cls.get_result(stmt, first=True)
...
#     run_type = mapped_column(
#         Integer, comment="运行类型， 10 同步， 20 异步，30 定时任务"
#     )
...
#         stmt = (
#             select(
#                 cls.get_table_columns(exclude={"duration"}),
...
#         return cls.query.filter(
#             cls.project_name == project_name, cls.enabled_flag == 1
#         ).first()
...
#         return cls.query.filter(
#             cls.enabled_flag == 1,
#             cls.start_at.between(begin_time, end_time),
...
#         return (
#             cls.query.outerjoin(User, User.id == cls.execute_user_id)
#             .with_entities(
#                 func.count(cls.id).label("run_num"),
#                 User.username.label("employee_code"),
#                 User.nickname.label("username"),
#             )
#             .filter(cls.enabled_flag == 1, cls.execute_user_id != -1, *q)
#         )
...
#             q.append(cls.creation_date.between(start_time, end_time))
#         return (
#             cls.query.filter(cls.enabled_flag == 1, cls.execute_user_id != -1, *q)
#             .outerjoin(ProjectInfo, ProjectInfo.id == cls.project_id)
#             .with_entities(
#                 cls.id,
#                 ProjectInfo.name.label("project_name"),
#                 func.round(
#                     func.sum(func.if_(cls.success, 1, 0)) / func.count(cls.id) * 100, 2
#                 ).label("pass_rate"),
#                 func.round(
#                     func.sum(cls.successful_use_case)
#                     / func.sum(cls.run_test_count)
#                     * 100,
#                     2,
#                 ).label("successes_rate"),
#             )
...
#                 name = mapped_column(
#                     String(255), nullable=False, comment="步骤名称", index=True
#                 )
#                 case_id = mapped_column(String(255), comment="用例id")
#                 success = mapped_column(Integer, comment="是否成功， True, False")
#                 status = mapped_column(
#                     String(255), comment="步骤状态  success 成功  fail 失败  skip 跳过"
#                 )
...
#                             cls.get_table_columns(),
#                             func.if_(
#                                 ApiTestReport.run_type != "api",
#                                 ApiTestReport.name.label("case_name"),
#                                 None,
#                             ),
#                             ApiInfo.name.label("api_name"),
#                             ApiInfo.created_by.label("api_created_by"),
#                             User.nickname.label("api_created_by_name"),
#                         )
#                         .where(*q)
#                         .outerjoin(ApiInfo, ApiInfo.id == cls.source_id)
...
#                             func.count(
#                                 func.if_(cls.case_id.is_not(None), 1, None)
#                             ).label("case_count_1"),
#                             func.sum(
#                                 func.if_(
#                                     and_(
#                                         cls.status == "SUCCESS",
#                                         cls.case_id.is_not(None),
#                                     ),
#                                     1,
#                                     0,
#                                 )
#                             ).label("case_step_success_count"),
#                             # 用例步骤成功数
#                             func.sum(
#                                 func.if_(
#                                     and_(
...
#                                     0,
#                                 )
#                             ).label("case_step_fail_count"),
#                             # 步骤 -------------------------------------------------
#                             # 总步骤数
#                             func.count("*").label("step_count"),
#                             func.count(func.if_(cls.status != "SKIP", 1, None)).label(
#                                 "effective_step_count"
#                             ),
#                             # 成功步骤数
#                             func.sum(
#                                 func.if_(
#                                     and_(cls.status == "SUCCESS", cls.status != "SKIP"),
#                                     1,
#                                     0,
#                                 )
#                             ).label("step_success_count"),
#                             # 失败步骤数
#                             func.sum(func.if_(cls.status == "FAILURE", 1, 0)).label(
#                                 "step_fail_count"
#                             ),
#                             # 跳过步骤数
#                             func.sum(func.if_(cls.status == "SKIP", 1, 0)).label(
#                                 "step_skip_count"
#                             ),
#                             # 错误步骤数
#                             func.sum(func.if_(cls.status == "ERROR", 1, 0)).label(
#                                 "step_error_count"
#                             ),
#                             # 平均请求时长
#                             func.round(
#                                 func.IFNULL(func.avg(cls.elapsed_ms), 0), 2
#                             ).label("avg_request_time"),
#                             # 总执行时长
#                             func.sum(cls.duration).label("request_time_count"),
#                         )
#                         .where(*q)
#                         .group_by(cls.case_id)
#                         .subquery()
#                     )

#                     stmt = select(
#                         func.count(
#                             func.if_(sub_stmt.c.case_id.is_not(None), 1, None)
#                         ).label("case_count"),
#                         func.sum(sub_stmt.c.step_count).label("step_count"),
#                         func.round(func.avg(sub_stmt.c.avg_request_time), 2).label(
#                             "avg_request_time"
#                         ),
#                         func.sum(sub_stmt.c.request_time_count).label(
#                             "request_time_count"
#                         ),
#                         func.sum(
#                             func.if_(
...
#                                     sub_stmt.c.case_step_fail_count == 0,
#                                 ),
#                                 1,
#                                 0,
#                             )
...
#                                 )
#                             )
#                         ).label("case_fail_count"),
#                         func.sum(sub_stmt.c.step_success_count).label(
#                             "step_success_count"
#                         ),
...
#                         func.round(
#                             func.sum(sub_stmt.c.step_success_count)
#                             / func.sum(sub_stmt.c.effective_step_count)
#                             * 100,
#                             2,
#                         ).label("step_pass_rate"),
#                         func.round(
#                             (
#                                 func.sum(
#                                     func.if_(sub_stmt.c.case_step_fail_count == 0, 1, 0)
#                                 )
#                             )
#                             / func.count(
#                                 func.if_(sub_stmt.c.case_id.is_not(None), 1, None)
...
#         Integer, nullable=False, index=True, comment="数据源id"
#     )
...
#         stmt = (
#             select(cls.get_table_columns(), ProjectInfo.name.label("project_name"))
#             .where(cls.enabled_flag == 1, cls.id == id)
#             .outerjoin(ProjectInfo, cls.project_id == ProjectInfo.id)
#         )
...
#         stmt = (
#             select(
#                 cls.get_table_columns(exclude={"password"}),
#                 u.nickname.label("created_by_name"),
#                 User.nickname.label("updated_by_name"),
#             )
#             .where(*q)
